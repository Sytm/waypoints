 ...
 * https://wiki.vg
 */
internal open class ClientSideEntity(
  ...
  private val spawnPacket
    get() =
        PacketContainer(PacketType.Play.Server.SPAWN_ENTITY).also {
          ...
          // Set angles to zero
          it.bytes.write(0, 0).write(1, 0).write(2, 0)

          // Set velocity to zero
          it.integers.write(1, 0).write(2, 0).write(3, 0)
        }

  protected open fun modifyMetadataValues(
      spawn: Boolean,
      dataValues: MutableList<WrappedDataValue>
  ) {}
  ...
  private fun getMetadataPacket(spawn: Boolean): PacketContainer? {
    ...
    return PacketContainer(PacketType.Play.Server.ENTITY_METADATA).also {
      it.integers.write(0, entityId)
      it.dataValueCollectionModifier.write(0, values)
    }
  }

  private val movePacket
    get() =
        PacketContainer(PacketType.Play.Server.REL_ENTITY_MOVE).also {
          it.integers.write(0, entityId)
  ...
  private val teleportPacket
    get() =
        PacketContainer(PacketType.Play.Server.ENTITY_TELEPORT).also {
          it.integers.write(0, entityId)

          it.doubles.write(0, location.x).write(1, location.y).write(2, location.z)

          // Set angles to zero
          it.bytes.write(0, 0).write(1, 0)
  ...
  private val requiresTeleport
    get() =
        abs(location.x - previousLocation.x) > 8 ||
            abs(location.z - previousLocation.z) > 8 ||
            abs(location.y - previousLocation.y) > 8
  ...
  private val destroyPacket =
      PacketContainer(PacketType.Play.Server.ENTITY_DESTROY).also {
