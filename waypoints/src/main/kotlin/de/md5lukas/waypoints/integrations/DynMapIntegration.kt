 ...
 */
class DynMapIntegration(private val plugin: WaypointsPlugin) : Listener {
    ...
    try {
      ...
      markerSet =
          markerApi.createMarkerSet(
              ...
              null,
              false) // id, label, iconlimit, persistent
      ...
      plugin.slF4JLogger.error(
          "The DynMap plugin has been found, but plugin instance class could not be found")
      return false
    }
    return true
  }
  ...
  fun onUpdate(e: WaypointCustomDataChangeEvent) {
    ...
    plugin.skedule {
      markerSet.findMarker(e.waypoint.id.toString())?.markerIcon =
          getMarkerForWaypoint(e.waypoint, e.data)
    }
  }
