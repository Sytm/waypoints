...
internal class WaypointsAPIImpl(
    ...
    override suspend fun getWaypointPlayer(uuid: UUID): WaypointsPlayer = withContext(dm.asyncDispatcher) {
        dm.instanceCache.playerData.get(uuid) {
    ...
    override suspend fun waypointsPlayerExists(uuid: UUID): Boolean = withContext(dm.asyncDispatcher) {
    ...
    override suspend fun getWaypointByID(uuid: UUID): Waypoint? = dm.connection.selectFirst("SELECT * FROM waypoints WHERE id = ?", uuid.toString()) {
